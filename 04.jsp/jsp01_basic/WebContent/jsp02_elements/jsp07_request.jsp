...
<!-- 
	...
	웹브라우저에서 웹사이트주소를 입력하면 웹브라우저는 해당주소의 웹서버에 연결한 후 요청 정보를 전송하는데
	이 요청정보를 웹서버에 전달하는 객체가 request객체이다.
	...
	A. request객체가 제공하는 기능
		...
		d. 클라이언트가 전송한 요청헤더정보(예:http통신규악에 따른 정보)
		e. 클라이언트가 전송한 쿠키정보
		f. 속성처리정보(properties)
	...
	B. request객체의 메서드
	    ...
	    d. getContentType()        : 클라이언트가 요청정보를 전송할 때 사용한 컨텐츠(문서)타입 정보
	    e. getProtocol()           : 클라이언트가 요청한 프로토콜(http,https,ftp...)
	    f. getMethod()             : 웹브라우저가 정보를 전송할 때의 방식(get,post...)
	    g. getRequestURI()         : 웹브라우저가 요청한 URL정보
	    h. getContextPath()        : jsp페이지가 소속된 웹애플리케이션의 ContexPath정보
	    i. getServerName()         : 연결할 때 사용한 웹서버의 이름
	    j. getSeverPort()          : 연결할 때 사용한 웹서버의 포트번호
		
	C. request객체의 요청파라미터 관련 메서드
	
		a. getParameter(String name)       : name인 파라미터의 값을 리턴, 없을 경우 null          - input...
		b. getParameterValues(String name) : name인 파라미터의 값을 배열로 리턴, 없을 경우 null   - 체크박스...
