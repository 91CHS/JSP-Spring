...
<!--
	...
	A. response객체의 메서드
		...
		f. setDateHeader(String name, long value) : name이라는 헤더에 long 타입의 date값을 저장
	...
	B. 웹브라우저에 Cache제어 관련 응답헤더
		...
		캐쉬(Cache)란? 웹브라우저가 WAS에 jsp실행을 요청하고 잠시 뒤에 동일한 JSP 실행을 요청하는
		경우에 첫 번째 요청과 두 번째 요청사이에 결과값이 차이가 없을 경우에 불필요한 응답결과를
		반복해서 요청하는 셈이 된다. 캐시는 이렇게 동일한 데이터를 중복해서 로딩하지 않도록 할 경우
        에 사용한다. 웹브라우저는 첫 번째 요청시에 응답결과를 로컨PC의 임시저장소인 캐시에 저장한다.
        
        이후에 동일한 URL에 대한 요청이 있을 경우에 WAS에 접근하지 않고 로컬PC에 저장된 응답결과를
        사용한다. 캐시에 보관된 데이터를 사용할 경우 서버에 접근하지 않기 때문에 훨씬 빠른 결과를
        웹브라우저에 출력할 수 있게 된다.
        
        따라서, 변경이 발생하지 않을 경우 jsp의 응답결과나 이미지, 정적인 html등을 캐시에 보관함
        으로써 웹브라우저의 응답속도를 향상 시킬 수가 있다.
				
	C. response객체가 제공하는 헤더추가 메서드
	
		a. Cache-Control : 이 헤더값을 "no-cache"로 지정하더라도 응답정보를 캐시에 저장할 수 있다.
							캐시에 저장하지 않도록 하려면 이 헤더의 값을 "no-store"를 추가
		b. Pragma        : 이 헤더값을 "no-cache"로 저장하면 웹브라우저는 캐시를 사용하지 않는다.
