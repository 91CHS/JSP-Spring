...
public class DBCPInit extends HttpServlet{
	...
	private void loadJDBCDriver() {
			...
			// A. Connection Factory 생성
			   // 1. 커넥션풀이 새로운 커넥션을 생성할 때 사용할 커넥션팩토리를 생성
			   ...
			   // 2. PoolableConnection을 생성
			      ...
			      // 하는데 필요한 기능을 제공한다.
			   	  // 예를들어 connection을 close하면 connection을 메모리에서 삭제하는 것이
				  // 아니라 Connection Pool에 Connection을 반환한다.
				  PoolableConnectionFactory pcf = new PoolableConnectionFactory(cf, null);
				
		   	   // 3. Connection이 유효한지 검사하기 위한 SQL을 저장
				   pcf.setValidationQuery("select 1"); // mairadb, mysql
			...
			// C. Connection Pool 설정정보를 이용해서 Connection Pool을 생성
		      // 1. PoolConnnection을 생성할 때 사용할 커넥션팩토리와 설정정보를
			  //    이용해서 커넥션풀을 생성
