...
<!-- 
	 ...
	 tomcat과 같은 컨테이너(웹서버)는 JSP를 분석하는 과정에서(~.java파일 생성) 어떤 코딩을
	 이용해서 코드를 작성했는지를 검사하고 그 결과를 선택한 케릭터셋을 이용해서 JSP페이지의
	 문자를 읽어 오게 된다. 웹컨테이너가 JSP페이지를 읽을 때 사용할 문자셋을 결정하는 과정은
	 아래와 같다.
	 
	 1. 파일이 ROM으로 시작하지 않을 경우
	 	...
	 	d. 상기 모두 해당되지 않을 경우에는 기본문자셋인 ISO-8859-1 문자셋을 사용
